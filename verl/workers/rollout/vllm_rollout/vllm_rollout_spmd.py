...
rollout_external_score_timing_file = os.getenv("VERL_ROLLOUT_EXTERNAL_SCORE_TIMING_FILE", "")
rollout_sentence_timing_file = os.getenv("VERL_ROLLOUT_SENTENCE_TIMING_FILE", "")
...
def _append_timing_jsonl(path: str, payload: dict):
    try:
        ...
        # Timing logging must not interrupt rollout.
        pass
...
def build_single_prompt(tokenizer, processor, context_str, max_prompt_length, truncation="right"):
    ...
    if input_ids.size(1) > 0:
        ...
        if eos_id is not None and last_token == eos_id:
            print(f'Warning: removed trailing EOS token in build_single_prompt. ID: {last_token}')
...
def debug_print_full_batch(batch, non_tensor_batch=None, tokenizer=None, max_elems=10):
    ...
    if non_tensor_batch is not None and len(non_tensor_batch) > 0:
        print('\nnon_tensor_batch keys (with contents):')
...
def _split_sentences(text, attach_trailing_whitespace=True):
    ...
    return tokens
def _get_label(api_url:str, context: str, text: str):
    if not text.strip():
        return "yes"
    try:
        data = ast.literal_eval(context)
        
        context = data[0]['context'].strip()
        # print("================")
        # print(context)
        # print("================")
        r = requests.post(
            api_url, json={"context": context, "sentence": text}, timeout=10000
        )
        return r.json().get("result", "").lower().strip()
    except Exception as e:
...
def _get_score(api_url: str, context: str, sentences: list[str]) -> str:
        ...
        _append_timing_jsonl(
            rollout_external_score_timing_file,
            {
                "source": "rollout_external_score",
...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # print(f"Batch size: {idx.shape[0]}, prompt_length: {idx.shape[1]}")
        ...
        #     left_pad_len = (attention_mask[i] == 0).nonzero(as_tuple=True)[0].numel()
        ...
        # print(non_tensor_batch["extra_context"])
        ...
        with self.update_sampling_params(**kwargs):
            rollout_generate_t0 = time.perf_counter()
            ...
            rollout_generate_elapsed = time.perf_counter() - rollout_generate_t0
            _append_timing_jsonl(
                rollout_sentence_timing_file,
                {
                    "source": "vllm_rollout",
                    "record_type": "rollout_generate",
                    "elapsed_s": rollout_generate_elapsed,
                    "batch_size": int(batch_size),
                    "num_prompts": len(vllm_inputs),
                    "step": int(step),
                    "timestamp": time.time(),
                },
            )
        ...
        # print(response)
        # print("================")
        # Assuming `inputs` and `output` are lists of token IDs (e.g., from your data)
        from transformers import AutoTokenizer
        from transformers import pipeline
        tokenizer = AutoTokenizer.from_pretrained(
            os.getenv("VERL_RESAMPLE_TOKENIZER", "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B"),
        ...
        for context,output_text in zip(non_tensor_batch["extra_context"],decoded_outputs):
            ...
            score=_get_score("http://localhost:8126/predict",context,cot_sents)
            # print(score)
            for s in score:
                # print(score)
                if s<0.5:
...
Answer:""".format(question=question,knowledge=context_new)
        ...
        if any(need_resample_list):
            ...
            for i, (need_resample, context_str) in enumerate(zip(need_resample_list, resample_context_list)):
                ...
                # position_ids_r = compute_position_id_with_mask(attention_mask_r)
            ...
            with self.update_sampling_params(**kwargs):
                resample_generate_t0 = time.perf_counter()
                ...
                resample_generate_elapsed = time.perf_counter() - resample_generate_t0
                _append_timing_jsonl(
                    rollout_sentence_timing_file,
                    {
                        "source": "vllm_rollout",
                        "record_type": "rollout_resample_generate",
                        "elapsed_s": resample_generate_elapsed,
                        "batch_size_before_resample": int(batch_size),
                        "num_resample_prompts": len(vllm_inputs_resample),
                        "step": int(step),
                        "timestamp": time.time(),
                    },
                )
            ...
            print(f'Merged resampled batch: original {batch_size}, added {len(resample_prompts)}, total {batch_size_new}')
        else:
            print('No resampling needed')
        ...
        if any(need_resample_list):
            ...
            print(
                f'Merged non_tensor_batch, including response_final. '
                f'original {batch_size}, '
                f'resampled samples {sum(1 for x in need_resample_list if x)}, '
                f'copies per resampled sample {repeat_times}, '
                f'total {len(next(iter(non_tensor_batch.values())))}'
            )
        else:
            print('No resampling needed')
